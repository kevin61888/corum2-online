...
DFXform::DFXform() : m_nKeySize(KEYBYTES)
{
...
void DFXform::UUPadAndEncode(const BYTE* pSrc, BYTE* pDes, UINT nPaddingSize)
{
    BYTE Zero   = 0;
    BYTE One    = 1;
...
void DFXform::UUPartialEncode(const BYTE* pSrc, BYTE* pDes)
{
...
void DFXform::UUPartialDecode(const BYTE* pSrc, BYTE* pDes)
{
...
void DFXform::XXPaddAndEncode(const BYTE* pSrc, BYTE* pDes, UINT nPaddingSize)
{
    BYTE Zero    = 0;
    BYTE One    = 1;
    register BYTE c1 =0, c2=0, c3=0, c4=0;
...
void DFXform::XXPartialEncode(const BYTE* pSrc, BYTE* pDes)
{
    register BYTE c1, c2, c3, c4;
...
void DFXform::XXPartialDecode(const BYTE* pSrc, BYTE* pDes)
{
    register BYTE c1, c2, c3;
...
void DFXform::Encode(LPBYTE pSrc, DWORD dwSrcSize, LPBYTE pDes, LPDWORD pDesSize)
{
	...
	if(btPaddingSize) memset(pDes + 1 + dwSrcSize, (BYTE)-1, btPaddingSize); 
...
void DFXform::Encode(LPBYTE pSrc, WORD wdSrcSize, LPBYTE pDes, LPWORD pDesSize)
{
	...
	// 우리는 (BYTE)-1로 메시지의 뒤를 패딩한다!
	if(btPaddingSize) memset(pDes+1 + wdSrcSize, (BYTE)-1, btPaddingSize); 
...
#ifdef _DEBUG
	// 잘못된 메시지를 걸러낸다.
	if(pSrc == NULL)		{*pDesSize = 0;return;}
	if(pDes == NULL)		{*pDesSize = 0;return;}
	if(dwSrcSize == 0)		{*pDesSize = 0;return;}
...
void DFXform::Decode(LPBYTE pSrc, WORD wdSrcSize, LPBYTE pDes, LPWORD pDesSize)
{
	...
	if(pSrc == NULL)		{*pDesSize = 0;return;}
	if(pDes == NULL)		{*pDesSize = 0;return;}
	if(wdSrcSize == 0)		{*pDesSize = 0;return;}
	if(pSrc[0] >= 8)		{*pDesSize = 0;return;}	// pSrc[0] == btPaddingSize
