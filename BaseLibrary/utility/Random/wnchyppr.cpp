...
#ifndef RANDOMA_H  // may use faster versions in assembly library
...
// The return value is < -127 for x < 0.
// This function relies on x being stored in the IEEE standardized format.
  int32 n = *(int32 *)&x;
  return (n >> 23) - 0x7F;}
...
CWalleniusNCHypergeometric::CWalleniusNCHypergeometric(int32 n_, int32 m_, int32 N_, double odds_, float accuracy_) {
  // constructor
  accuracy = accuracy_;
...
double CWalleniusNCHypergeometric::recursive() {
  ...
  float accuracya;                    // absolute accuracy
...
double CWalleniusNCHypergeometric::laplace() {
  ...
  PrecisionIndex = (-FloorLog2(accuracy) - ERFRES_B + ERFRES_S - 1) / ERFRES_S;
...
int CWalleniusNCHypergeometric::MakeTable(double * table, int32 MaxLength, int32 * start, int32 * end) {
  ...
  if (area < 4000 || area < 10000 && N > 1000.*n) {
    ...
    *start = x1;  *end = x1 + i2 - 1;
    ...
    ONE_BY_ONE:
    accuracya = 0.01f * accuracy;      // absolute accuracy
...
CMultiWalleniusNCHypergeometric::CMultiWalleniusNCHypergeometric(int32 n_, int32 * m_, double * odds_, int colors_, float accuracy_) {
  // constructor
  accuracy = accuracy_;
