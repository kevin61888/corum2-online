...
StochasticLib3::StochasticLib3(int seed) : StochasticLib1(seed) {
  SetAccuracy(1.E-8f);}  // set default accuracy


/***********************************************************************
             SetAccuracy
***********************************************************************/
void StochasticLib3::SetAccuracy(float accur) {
  // define accuracy of calculations for 
  // WalleniusNCHyp and MultiWalleniusNCHyp
  if (accur < 0.) accur = 0.;
  if (accur > 0.01f) accur = 0.01f;
  accuracy = accur;}
...
int32 StochasticLib3::WalleniusNCHypRatioOfUnifoms (int32 n, int32 m, int32 N, double odds) {
  ...
  if (n != wnc_n_last || m != wnc_m_last || N != wnc_N_last || odds != wnc_o_last) {
    ...
    if (!UseChopDown) {
      ...
      if ((odds>5. || odds<0.2) && r1>=-0.5 && r1<=8.) {
        ...
        if (r1 < 1.) r1 = 1.;
        s4 = 0.029 * pow((double)N,(double)0.23) / (r1*r1);}
      wnc_h = 2. * (s123 + s4);
  ...
  while(1) {                                     // rejection loop
    ...
    if (xi < wnc_bound1 || xi > wnc_bound2) {
      ...
      if (wnch.BernouilliH(xi, hx * wnc_k * 1.01f, u * u * wnc_k  * 1.01f, this)) {
        break;}                                  // acceptance
    #endif      
    }                                            // rejection
  return xi;}
...
*/
  ...
  int32 x0, x;                 // sample of one color
